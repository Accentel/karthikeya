...
<script type="text/javascript">
							...
							console.log("Unit Cost (" + id + "):", srate1);
							if(srate1=='' || srate1==null){srate1=0;}
							var totqty=document.getElementById("bal"+id).value;
						  	console.log("balance("+id+"):", totqty)
							if(totqty=='' || totqty==null){totqty=0;}
							if(str>eval(totqty)){
								alert("Please Enter Sale Quantity Below "+totqty);
								document.getElementById("sqty"+id).value="";
								return false;
							}
							//alert("srate1--"+srate1)
							//alert("str--"+str)
							var prev=document.getElementById("value"+id).value;
							console.log("value("+id+"):", prev);
							var srate2=0;
							...
							console.log("Discount("+id+"):", doscount);
 ...
 var totaldiscount=0;
	...
	for(i=1;i<=count;i++)
	{
		...
		// document.getElementById("disc").value = totaldiscount.toFixed(2);
		document.getElementById("disc").value= (totaldiscount);
		// document.getElementById("nettotal").value = sum.toFixed(2);	
		document.getElementById("nettotal").value= (sum);
		//  document.getElementById("amount").value = (sum - totaldiscount).toFixed(2);
                 document.getElementById("amount").value= (sum);
				  document.getElementById("grand").value=(sum);
				  document.getElementById("final").value= (sum);
	}
...
function saletype(str){
   ...
   function funconce2(str){
	...
	if(str == "Yes"){
		...
		<script>
			function roundFinalAmount() {
            var finalAmount = parseFloat(document.getElementById('final').value) || 0;
            var roundedAmount = Math.round(finalAmount);
            document.getElementById('final').value = roundedAmount.toFixed(2);

			var netAmount = parseFloat(document.getElementById('nettotal').value) || 0;
            var roundedAmount = Math.round(finalAmount);
            document.getElementById('nettotal').value = roundedAmount.toFixed(2);

			var grandAmount = parseFloat(document.getElementById('grand').value) || 0;
            var roundedAmount = Math.round(finalAmount);
            document.getElementById('grand').value = roundedAmount.toFixed(2);

			var payAmount = parseFloat(document.getElementById('amount').value) || 0;
            var roundedAmount = Math.round(finalAmount);
            document.getElementById('amount').value = roundedAmount.toFixed(2);	
        }
		</script>
...
NotPaid</td>-->
                          ...
                          <tr>
							...
							<tr id="cashtr" style="display:''">
							  ...
							  <td align="left">
							  <input name="disc" type="text" class="text" id="disc" value="0" oninput="nett(this.value)" />
					</td>
					</tr>
    ...
    $('#amount2').val(totalpoints - totalpoints1);
	  ...
	  <input name="actb" id="actb"  type="hidden" value="1"/>
	   <input name="rowval" id="rowval"  type="hidden" value="0"/>
                    ...
                    <td ><div align="center">
					<input type="button" value="Round" onclick="roundFinalAmount()">
...
<head>
    ...
    <title>Auto Calculate Total and Final Amount for Multiple Records</title>
    <script>
        function calculateRowTotal(rowIndex) {
            var price = parseFloat(document.getElementById('price_' + rowIndex).value) || 0;
            var quantity = parseFloat(document.getElementById('quantity_' + rowIndex).value) || 0;
            var totalAmount = price * quantity;
            document.getElementById('total_amount_' + rowIndex).value = totalAmount.toFixed(2);

            // Calculate individual discount and update
            var discountPercentage = parseFloat(document.getElementById('discount_' + rowIndex).value) || 0;
            var individualDiscount = (totalAmount * discountPercentage) / 100;
            document.getElementById('individual_discount_' + rowIndex).value = individualDiscount.toFixed(2);

            // Calculate individual total amount after discount
            var individualTotalAmount = totalAmount - individualDiscount;
            document.getElementById('individual_total_amount_' + rowIndex).value = individualTotalAmount.toFixed(2);
        ...
        function calculateFinalAmount() {
            ...
            // Loop through each row to calculate the total amount and final amount after discount
            for (var i = 1; i <= rowCount; i++) {
                var totalAmount = parseFloat(document.getElementById('total_amount_' + i).value) || 0;
                var individualTotalAmount = parseFloat(document.getElementById('individual_total_amount_' + i).value) || 0;
                var individualDiscount = parseFloat(document.getElementById('individual_discount_' + i).value) || 0;

                totalSum += totalAmount; // Sum of all total amounts
                finalTotalSum += individualTotalAmount; // Sum of all final amounts after discount
                totalDiscountAmount += individualDiscount; // Sum of all individual discounts
            }

            // Calculate the average discount percentage based on total discount amount
            var averageDiscountPercentage = (totalDiscountAmount / totalSum) * 100;

            // Display the total sum, final amount and average discount
            document.getElementById('total_sum').value = totalSum.toFixed(2);
            document.getElementById('final_amount').value = finalTotalSum.toFixed(2); // Final total after applying individual discounts
            document.getElementById('average_discount').value = averageDiscountPercentage.toFixed(2);
        }

        function roundRowTotal(rowIndex) {
            var individualTotalAmount = parseFloat(document.getElementById('individual_total_amount_' + rowIndex).value) || 0;
            var roundedAmount = Math.round(individualTotalAmount);
            document.getElementById('individual_total_amount_' + rowIndex).value = roundedAmount.toFixed(2);
            calculateFinalAmount(); // Recalculate the final amount after rounding
        }

        function roundFinalAmount() {
            var finalAmount = parseFloat(document.getElementById('final_amount').value) || 0;
            var roundedAmount = Math.round(finalAmount);
        ...
        function addRecord() {
            ...
            row.innerHTML = `
                <td><input type="number" id="price_${rowCount + 1}" name="price[]" step="0.01" oninput="calculateRowTotal(${rowCount + 1})"></td>
                <td><input type="number" id="quantity_${rowCount + 1}" name="quantity[]" oninput="calculateRowTotal(${rowCount + 1})"></td>
                <td><input type="number" id="total_amount_${rowCount + 1}" name="total_amount[]" readonly></td>
                <td><input type="number" id="discount_${rowCount + 1}" name="discount[]" step="0.01" oninput="calculateRowTotal(${rowCount + 1})"></td>
                <td><input type="number" id="individual_discount_${rowCount + 1}" name="individual_discount[]" readonly></td>
                <td><input type="number" id="individual_total_amount_${rowCount + 1}" name="individual_total_amount[]" readonly></td>
                <td><button type="button" onclick="roundRowTotal(${rowCount + 1})">Round</button></td>
            `;
        }
    </script>
</head>
<body>
    ...
    <form action="process.php" method="POST">
        <table id="records_table" border="1">
            <thead>
                <tr>
                    ...
                    <th>Individual Total Amount</th>
                    <th>Round</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td><input type="number" id="price_1" name="price[]" step="0.01" oninput="calculateRowTotal(1)"></td>
                    <td><input type="number" id="quantity_1" name="quantity[]" oninput="calculateRowTotal(1)"></td>
                    <td><input type="number" id="total_amount_1" name="total_amount[]" readonly></td>
                    <td><input type="number" id="discount_1" name="discount[]" step="0.01" oninput="calculateRowTotal(1)"></td>
                    <td><input type="number" id="individual_discount_1" name="individual_discount[]" readonly></td>
                    <td><input type="number" id="individual_total_amount_1" name="individual_total_amount[]" readonly></td>
                    <td><button type="button" onclick="roundRowTotal(1)">Round</button></td>
                </tr>
            </tbody>
        </table>
        <br>
        ...
        <input type="number" id="total_sum" name="total_sum" readonly>
        <br><br>

        <label for="average_discount">Average Discount (%):</label>
        <input type="number" id="average_discount" name="average_discount" readonly>
        <br><br>

        <label for="final_amount">Final Amount (Sum of individual totals):</label>
        <input type="number" id="final_amount" name="final_amount" readonly>
        <br><br>

        <label for="round_final_amount">Round Final Amount:</label>
        <input type="number" id="round_final_amount" name="round_final_amount" readonly>
        <input type="button" value="Round Final Amount" onclick="roundFinalAmount()">
        <br><br>
